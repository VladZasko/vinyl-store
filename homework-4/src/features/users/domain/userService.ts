...
export class UsersService {
    ...
    async createUser(createData: CreateUserModel): Promise<UserViewModel> {
        ...
        const passwordHash: string = await this._generateHash(createData.password, passwordSalt)
        ...
        const createResult: UserViewModel = await this.usersRepository.createUser(newUser)
    ...
    async checkCredentials(email: string, password: string): Promise<UserViewModel | null> {
        const user: UserType | undefined = await UsersRepository.findByEmail(email)
        if (!user) {
            return null
        }

        const passwordHash: string = await this._generateHash(password, user.passwordHash)
